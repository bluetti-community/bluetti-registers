...
# - d_serial: no override at all any more - it's genuinely 4 registers like
#   Balco260, using the same "serial" content the generic "_serial" suffix
#   rule already assigns by default. A real AC500's serial plate reads
#   "AC5002241000084770", but an earlier version of this override
#   (incorrectly) forced d_serial down to a single UINT16 register, which
#   only ever read register 50206 in isolation (0x9522 = 38178) - a
#   plausible-looking but incomplete number. Reading the full 4-register
#   block (50206-50209) with the same word_order="little" every other
#   "serial" field already uses decodes directly to 2241000084770 - the
#   exact digits from the real serial plate - with no extra swapping
#   needed (bluetti-official/bluetti-modbus-tcp-slave#5).
